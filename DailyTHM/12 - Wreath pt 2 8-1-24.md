- [x] Task 5: Enumeration
	How many of the first 15000 ports are open on the target?
		[[Nmap]] IP -p 0-15000
		4 ports
	What OS does Nmap think is running?
		nmap 10.200.84.200 -sV 
		centos
	Open the IP in your browser -- what site does the server try to redirect you to?
		https://thomaswreath.thm/
	You will have noticed that the site failed to resolve. Looks like Thomas forgot to set up the DNS! Add it to your hosts file manually. This can be accomplished by editing the `/etc/hosts` 
	/
	Read through the text on the page. What is Thomas' mobile phone number?
		+447821548812 
	Let's have a look at the highest open port.  Look back at your service scan results: what server version does Nmap detect as running here?
		MiniServ 1.890 (Webmin httpd)
	Putting this into google we can find this exploit
		https://www.rapid7.com/db/vulnerabilities/http-webmin-cve-2019-15107/

- [x] Task 6: exploit
	using the found expoit and https://github.com/MuirlandOracle/CVE-2019-15107 we can get a reverse shell.
	Who are we logged in as
		root
	What is the password hash for root
		`root:$6$i9vT8tk3SoXXxK2P$HDIAwho9FOdd4QCecIJKwAwwh8Hwl.BdsbMOUAd3X/chSCvrmpfy.5lrLgnRVNq6/6g0PxK9VqSdy47/qKXad1::0:99999:7:::
	We can get an ssh key from /root/.ssh/id_rsa
		```
```-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY----```
